...
impl CompoundValueSerializerContext {
  ...
  pub fn write_null(&mut self, scalar_field_specification: &ScalarFieldSpecification) -> Result<(), GenericError> {
    self.write_column(scalar_field_specification, "NULL");
    Ok(())
  }
  
  pub fn write_boolean(&mut self, scalar_field_specification: &ScalarFieldSpecification, boolean: bool) -> Result<(), GenericError> {
