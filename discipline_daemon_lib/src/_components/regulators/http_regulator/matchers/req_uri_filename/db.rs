...
pub fn add_one(matcher: &Matcher, session: &mut impl Write) {
  ...
  writeln!(session, "INSERT INTO http_uri_filename_matchers VALUES ({id}, {child_id}, {child_kind});").unwrap();
}

pub fn find_all(session: &mut impl Write) {
  writeln!(session, "SELECT * FROM http_uri_filename_matchers;").unwrap();
}
