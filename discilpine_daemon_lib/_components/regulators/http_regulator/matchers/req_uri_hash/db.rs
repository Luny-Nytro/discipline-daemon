...
pub fn add_one(matcher: &Matcher, session: &mut impl Write) {
  ...
  writeln!(session, "INSERT INTO http_uri_hash_matchers VALUES ({id}, {child_id}, {child_kind});").unwrap();
}

pub fn find_all(session: &mut impl Write) {
  writeln!(session, "SELECT * FROM http_uri_hash_matchers;").unwrap();
}
...
pub fn update_one(prev: &Matcher, curr: &Matcher, session: &mut impl Write) {
  ...
  match (&prev.child, &curr.child) {
    ...
    (None, Some(child)) => {
      updates.add("child_id", &child.id().serialize());
      updates.add("child_kind", &child.kind().serialize());
    }
    (Some(_), None) => {
  ...
  if updates.is_not_empty() {
    ...
    writeln!(session, "UPDATE http_uri_hash_matchers SET {updates} WHERE id = {id} ;").unwrap();
  }
}

pub fn delete_one(id: &Uuid, session: &mut impl Write) {
  let id = id.serialize();
  writeln!(session, "DELETE FROM http_uri_hash_matchers WHERE id = {id} ;").unwrap();
}