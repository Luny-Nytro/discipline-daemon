...
pub struct PolicySpecification {
  ...
  pub creation_time_field_specification: ScalarFieldSpecification,
}
...
impl PolicySpecification {
    ...
    Result<Self, GenericError>
  {
    ...
    let creation_time_field_specification = fields_namespace
      .scalar_field_specification("CreationTime")
      .build()
      .map_err(|error| error.change_context("creating PolicySpecification"))?;
    ...
    Ok(Self {
       creation_time_field_specification,
...
impl<'a> PolicySerializer<'a> {
  pub fn new(user_id: &'a Uuid, policy_schema: &'a PolicySpecification) -> Self {
    Self {
      user_id,
      policy_specification: policy_schema,
    }
  }
}
...
impl<'a> CompoundValueSerializer for PolicySerializer<'a> {
    ...
    Result<(), GenericError>
  {
    ...
    context.serializable_scalar(&self.policy_specification.creation_time_field_specification, &value.creation_time)?;
...
impl CompoundValueDeserializer for PolicySpecification {
  ...
  fn deserialize(&self, context: &CompoundValueDeserializerContext) -> Result<Self::Output, GenericError> {
    Ok(NormalizedPolicy {
      ...
      creation_time: context.deserializable_scalar(&self.creation_time_field_specification).map_err(|error|
        error
          .change_context("deserializing NormalizedPolicy")
          .add_error("failed to deserialize the 'CreationTime' field")
      )?,
    })
  }
}
...
impl NormalizedPolicy {
  pub fn denormalize(self, normalized_rules: &Vec<NormalizedRule>) -> Policy {
    Policy {
      id: self.id,
      name: self.name,
      rules: normalized_rules.iter().map(|rule| rule.clone().denormalize()).collect(),
