...
export type Error = Unique<"Discipline.NetworkingAccess.Operations.Activator.InWeekdayRange.Modify.Error", Variant>
...
function UnknownVariantError(string: string): GenericError.GenericError {
  return GenericError.create(
    `NetworkingAccess.Operations.Activator.InWeekdayRange.Modify.Error.fromString: Unknwn Error Variant. Expected ${
...
export function fromString(variantName: string): Tried<Error, GenericError.GenericError> {
  switch (variantName) {
    ...
    default: {
      return Err(UnknownVariantError(variantName))
    }
  }
}
...
export const displayer = Displayer.implement<Error>(me => 
  Displayer.asWrappedString("Activator.InWeekdayRange.Modify.Error", asString(me))
)
