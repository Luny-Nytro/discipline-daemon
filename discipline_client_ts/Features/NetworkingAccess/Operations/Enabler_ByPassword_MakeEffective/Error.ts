...
export interface Cases<A, B, C, D, E, F> {
  ...
  readonly AlreadyEffective: () => C
  readonly WrongPassword: () => F
  readonly InternalError: () => E
}

export type Error = Unique<"Discilpine.NetworkingAccess.Operations.Enabler.ByPassword.MakeEffective.Error", Variant>
...
const ALREADY_EFFECTIVE_AS_STRING = "AlreadyEffective"
...
export function AlreadyEffective(): Error {
  return Unique(Variant.AlreadyEffective)
}
...
function UnknownVariantError(variant: string): GenericError.GenericError {
  return GenericError.create(
    `NetworkingAccess.Operations.Enabler.ByPassword.MakeEffective.Error.fromString: Unknown variant. Expected ${
      ...
      Displayer.stringDisplayer.display(ALREADY_EFFECTIVE_AS_STRING)
    }, ${
...
export function fromString(variant: string): Tried<Error, GenericError.GenericError> {
  switch (variant) {
    ...
    case ALREADY_EFFECTIVE_AS_STRING: {
      return Ok(AlreadyEffective())
    }
...
export function asString(me: Error): string {
  return match(me, {
    ...
    AlreadyEffective: () => ALREADY_EFFECTIVE_AS_STRING,
...
export const displayer = Displayer.implement<Error>(me => 
  Displayer.asWrappedString("Enabler.ByPassword.MakeEffective.Error", asString(me))
)
