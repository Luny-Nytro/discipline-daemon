...
export interface Cases<A, B, C> {
  ...
  SomeRulesAreEnabled: () => B
  InternalError: () => C
}

export type Error = Unique<"Discipline.NetworkingAccess.Operations.RuleEnforcer.Delete.Error", Variant>
...
function UnknownVariantError(variant: string): GenericError.GenericError {
  return GenericError.create(
    `NetworkingAccess.Operations.RuleEnforcer.Delete.Error.fromString: Unknown variant. Expected ${
...
export const displayer = Displayer.implement<Error>(me => 
  Displayer.asWrappedString("DeleteRuleEnforcerError", asString(me))
)
