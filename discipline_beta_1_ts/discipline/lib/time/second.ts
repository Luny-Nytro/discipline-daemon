...
export class Second {
  ...
  static new0(value: Value0) {
    return new Second(value + 1 as Value1)
  }

  static new0Checked(value: number) {
    if (Number.isSafeInteger(value) && value >= 0 && value <= 59) {
      return new Some(new Second(value + 1 as Value1))
    } else {
      return new None()
    }
  }
